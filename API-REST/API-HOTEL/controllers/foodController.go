...
func CreateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			msg := fmt.Sprintf("room was not found")
		...
		if insertErr != nil {
			msg := fmt.Sprintf("hotel item was not created")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
			return
		}
		defer cancel()
...
func UpdateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if food.Menu_id != nil {
			...
			defer cancel()
			if err != nil {
				msg := fmt.Sprintf("message: Room was not found")
				c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
				return
			}
		...
		if err != nil {
			msg := fmt.Sprintf(" hotel item update failed")
			c.JSON(http.StatusInternalServerError, gin.H{"error": msg})
		}
		c.JSON(http.StatusOK, result)
	}
